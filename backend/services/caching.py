...
from datetime import (
    datetime,
    timedelta,
)  # (to work with time and date... datetime-> present time....timedelta-> time addition or subtraction  )
...
class HybridCache:
    ...
    def get(self, key):  # To retrieve data from the cache.
        ...
        try:
            ...
            if datetime.now() - mod_time > timedelta(
                hours=expiry_hours
            ):  # current time > expiry time then cache file will be deleted
            ...
            data = cache_data[
                "data"
            ]  # if cache is valid then stored data will be returned otherwise it will return null
